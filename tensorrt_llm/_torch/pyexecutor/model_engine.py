...
class PyTorchModelEngine(ModelEngine):
    ...
    def _run_autotuner_warmup(self, resource_manager: ResourceManager):
        ...
        # (num_tokens, num_gen_requests) warmup configs. The context-only
        # forward tunes ops keyed on the token count (MoE, GEMMs). The
        # generation-only forward tunes the decode attention path, which
        # never runs in a context batch: the target step at
        # seq_len_q = 1 + max_total_draft_tokens plus, with speculative
        # decoding, the draft steps at seq_len_q = 1 inside the same forward.
        warmup_configs = [(curr_max_num_tokens, 0)]
        if (not self.is_draft_model and self.guided_decoder is None
                and not self.mapping.has_pp()):
            # 2-model speculative draft and guided decoding do not support
            # dummy generation requests (see _run_attention_warmup). Under
            # PP, stick to the single context forward: a per-rank skip of
            # the extra forward would deadlock the pipeline.
